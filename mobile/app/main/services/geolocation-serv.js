...
.factory('Geolocation', function($cordovaGeolocation, $log, $q, $http) {
    ...
    */
    function get() {
        $log.log('Getting current location from device...');
        var defer = $q.defer();
        var options = {
            timeout: 10000,
        ...
        .then(
            ...
            function(error) {
                // No cordova? Let's try HTML5 geolocation.
                navigator.geolocation.getCurrentPosition(function(position) {
                    $log.log('Got location with HTML5:', position);
                    defer.resolve(position);
                },
                // Use hardcoded default if all fails.
                function(error) {
